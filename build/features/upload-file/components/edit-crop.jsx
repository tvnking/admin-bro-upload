...
const cropper_model_1 = __importDefault(require("./cropper-model"));
const EditCrop = ({ property, record, onChange }) => {
    const { params } = record;
    ...
    const [fileCrop, setFileCropp] = react_1.useState([]);
    react_1.useEffect(() => {
    ...
    const onUploadCrop = (files) => {
        setFileCropp(files);
    };
    const onUpload = (file) => {
        setFilesToUpload([file]);
        onChange(custom.fileProperty, [file]);
    };
    const handleRemove = () => {
        onChange(custom.fileProperty, null);
    };
    const handleMultiRemove = (singleKey) => {
        const index = (admin_bro_1.flat.get(record.params, custom.keyProperty) || []).indexOf(singleKey);
        const filesToDelete = admin_bro_1.flat.get(record.params, custom.filesToDeleteProperty) || [];
        if (path && path.length > 0) {
            const newPath = path.map((currentPath, i) => (i !== index ? currentPath : null));
            let newParams = admin_bro_1.flat.set(record.params, custom.filesToDeleteProperty, [...filesToDelete, index]);
            newParams = admin_bro_1.flat.set(newParams, custom.filePathProperty, newPath);
    ...
    return (<design_system_1.FormGroup>
      ...
      <design_system_1.DropZone onChange={onUploadCrop} multiple={custom.multiple} validate={{
        mimeTypes: custom.mimeTypes,
        maxSize: custom.maxSize,
    }} files={filesToUpload}/>
      <cropper_model_1.default file={fileCrop[0]} onConfirm={(croppedFile) => onUpload(croppedFile)} onCompleted={() => setFileCropp([])} onDiscard={() => { }} crop={custom.crop}/>
