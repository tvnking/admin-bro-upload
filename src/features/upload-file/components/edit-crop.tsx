...
const EditCrop: FC<EditPropertyProps> = ({ property, record, onChange }) => {
  const { params } = record
  ...
  const [fileCrop, setFileCropp] = useState<Array<File>>([])
  ...
  const onUploadCrop = (files: Array<File>): void => {
    setFileCropp(files)
  }
  ...
  return (
    <FormGroup>
      ...
      <DropZone
        ...
        files={filesToUpload}
      />
      <CropperModel file={fileCrop[0]}
        onConfirm={(croppedFile: File) => onUpload(croppedFile)}
        onCompleted={() => setFileCropp([])}
        onDiscard={() => { }}
        crop={custom.crop} />
